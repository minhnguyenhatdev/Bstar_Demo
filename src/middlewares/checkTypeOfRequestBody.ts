...
import { UserQueryRequest } from 'src/types/UserQueryRequest';
...
export const validateInputForProductQuery = async(req: IRequest<ProductQueryRequest>, res: IResponse<ResponseHandler<IStatusesResponse>>, next: NextFunction) => {
  try {
    ...
    let searchInput = req.query.searchInput ? req.query.searchInput as string : ""
    const lenght = await Product.count({where: {
...
export const validateInputForUserQuery = async(req: IRequest<UserQueryRequest>, res: IResponse<ResponseHandler<IStatusesResponse>>, next: NextFunction) => {
  try {
    //initializing queries value
    let pageLimit = 10
    let pageIndex = 1
    ...
    //count number of product in db
    let searchInput = req.query.searchInput ? req.query.searchInput as string : ""
    const lenght = await User.count({where: {
      name: Like(`%${searchInput}%`)
    }})
    let pageTotal =  lenght/pageLimit
